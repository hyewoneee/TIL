...
WebRTC(Web-Real-Time Communication)는 이제까지 소개한 프로토콜과는 다른다.

지금까지 소개한 것은 모두 브라우저와 서버의 통신에 이용되는 프로토콜이었다.

WebRTC는 브라우저/서버 간 통신뿐만 아니라 화상 전화 등 실시간 커뮤니케이션을 실현하는 기반으로서 설계됐다.

실현하고자 하는 애플리케이션도 다른 프로토콜과는 크게 다르기 때문에, 사용되는 기능도 많이 바뀐다.

통신 기반에 사용하는 트랜스포트층은 재전송 처리를 해주는 TCP가 아니라 오류처리나 재전송 처리를 하지 않는 UDP를 메인으로 사용한다.

또 P2P 통신이므로 상대 브라우저를 찾는 시그널링, 라우터 내부의 사설 주소만 있는 컴퓨터에서 동작하는 브라우저를 위해 NAT을 넘어 사설 주소만으로 통신할 수 있는 기술도 사용된다.

각 구성 기술은 처음부터 WebRTC용으로 개발된 것이 아니라, 예전부터 화상 전화용으로 개발된 기술이나 실시간 TLS 암호화를 추가한 데이터그램 통신 등 기존 기술에 바탕을 두고 각각 새롭게 되살려 조합했다.

### WebRTC 활용 사례(1)

RFC 7478을 보면 어떤 경우에 대응하고 싶은지 정리되어 있다.

WebRTC의 요소 기술은 웹소켓만큼 단순하지 않아 조합해서 여러 가지 활용 사례에 대응할 수 있다.

P2P를 사용한 활용 사례를 소개하겠다

- 단순한 화상통화 시스템
    - 웹 브라우저가 직접 다른 브라우저와 연결되는 것이 아니라 웹 서비스가 기점이 된다.
    - 통신 프로토콜의 하위 계층인 트랜스포트 층에는 다른 HTTP 통신 프로토콜과 마찬가지로 TCP(스트림형 소켓)를 사용할 수도 있지만 주로 UDP(데이터그램형 소켓)를 사용
    - 이는 재전송 처리나 통신량 제한 등을 직접 구현해야 하는 프로토콜이다.
    - 단 통신 경로는 UDP용 TLS인 DTLS로 암호화된다.
- 방화벽과 화상 통화
    - 일부 사용자가 UDP를 전송할 수 없는 방화벽 너머에서 사용할 수 있다.
    - 일부 사용자가 HTTP 프록시 경유 통신만 허용되는 방화벽 너머에서도 사용할 수 있다.
- 글로벌 네트워크에서의 화상 통화
    - 서비스를 실현하기 위해서는 상호 연결 설정(ICE, 전화교환기 같은 것)이 필요해진다.
    - 사용자 ID 등 어떤 정보를 바탕으로 상대를 찾아낸다. 또한 NAT 안쪽에 있어도 통화할 수 있도록 `STUN(Session Traversal Utilities for NAT)`, `TURN(Traversal Using Relay NAT)` 과 같은 NAT의 벽을 넘기 위한 기술도 필요할지 모른다.
- 엔터프라이즈 환경에서의 화상통화
    - 엔터프라이즈 용도에서는 회사 안에서 회사 밖으로의 통화를 관리할 필요가 있다.
    - 그러므로 내부 네트워크와 외부 네트워크를 연계하는 `TURN` 서버를 설치
    - 방화벽은 감시되는 `TURN` 서버를 사용하지 않는 통신을 차단한다.
- 스크린 공유
- 파일 교환
- 하키 경기 뷰터
- 여러 사람과의 화상회의
...
- 고객센터
    - 클라이언트는 브라우저 대 브라우저가 아니라 서비스를 하는 웹 서버와 세션을 확립해서 통화한다
- IP 전화
    - 웹 브라우저를 사용해 일반 전화번호로 전화를 걸 수 있는 서비스를 구현할 수 있다.
    - 실제 전화망에 접속하는 것은 서버가 하지만 사용자는 브라우저를 이용해 서비스를 이용한다.
    - 웹 서비스 수준에서 이 장치에 응답하기 위한(예를 들어 `요금에 관한 문의는 1번을 눌러주세요`) DTMF 신호를 보낼 수 있어야 한다.
- 중앙 서버를 이용한 화상회의 시스템
    - 중앙 서버를 거쳐서 통신
    - P2P에서는 다대다 세션 수가 단번에 늘어난다.
    - 중앙 서버를 거치면 각 브라우저가 송신할 곳이 적어져 세션 수를 줄일 수 있다.

### RFC 이외의 활용 사례

1방향 스트림도 사용할 수 있다.

일대다 실시간 영상 배포에도 쓸 수 있다.

라이브 방송에서 `WebRTC`의 장점은 실시간성이 높다는 거다.

### RTCPeerConnection

WebRTC의 기술 요소를 설명하면서 브라우저에서 사용되는 API fksms rhkswjadptj wjdflgksek

- RTCPeerConnection: 통신 경로 확보와 미디어 채널의 오픈
- mdiaDevices.getUserMedia: 카메라, 마이크와 동영상 , 음성 핸들링
- RTCDataChannel: 데이터 채널 통신

WebRTC의 기반이 된 것은 IP 전화다.

IP 전화에서 사용되는 기술을 하나로 모아, 자바스크립트의 API를 정했다.

NAT 통과 등 기초를 이루는 프로토콜은 모두 예전부터 사용되던 것이다.

**SDP**

세션 기술 프로토콜(Session Description Protocol SDP)는 P2P 니고시에이션 시에 서로의 IP 주소와 포트양쪽에서 이용할 수 있는 오디오와 동영상 코딩 정보를 공유하기 위한 프로토콜이다.

**ICE**

상호 연결 설정(Interactive Connectivity Establishment ICE)는 NAT를 넘어 P2P 전송 연결을 설정하는 방법이다.

STUN과 TURN중 한 서버를 이용한다.

라우터 등 글로벌 네트워크와 격리된 환경에서 각 컴퓨터는 로컬 주소밖에 모른다.

라우터 외부에 엑세스하면, NAT가 외부 컴퓨터와 내부 컴퓨터의 주소와 포트를 각각 매핑한다.

이것은 통신 응답을 제대로 받기 위한 구조이지만, 이를 이용해 밖에서 안으로 들어오는 통신 경보를 확보한다.

로컬 컴퓨터가 이 표적 대신 패킷을 보내는 곳이 `STUN` 서버다.

`STUN` 서버는 요청을 한 글로벌 IP주소와 포트를 NAT 안쪽의 컴퓨터로 반송한다.

이 두 단계로 통신 경로를 확보하고 외부에서 통신할 수 있는 주소와 포트를 얻을 수 있다.

이 주소와 포트를 상대에게 전달함으로써 NAT안에서 P2P 접속이 가능해진다.

STUN 서버로 통신할 수 없을 때는 주로 TCP 등으로 대체되지만, 그런 떄는 TURN 서버가 통신을 중계한다.

이 경우 웹소켓과 유사한 통신 형태다.

### 미디어 채널과 getUserMedia

상대와의 통신이 연결되면 실제로 통신한다.

이때 음성이나 비디오를 다루는 것이 미디어 채널이다.

**naviagator.mediaDevices.getUserMedia()**

화상회의에 사용하는 웹 카메라와 오디오 장치의 설정과 취득을 하는 브라우저 API이다.

카메라의 경우는 상도, 프레임레이트, 전면 카메라/후면 카메라 등을 지정할 수도 있다.

취득된 것은 스트림이라는 이름으로 불린다.

**DTLS**

`WevRTC` 의 데이터 통신에는 미디어 채널과 데이터 채널 두 가지가 있다.

양쪽 다 `DTLS` 상의 통신이다.

`DTLS` 는 `D: 데이터그램` 의 TLS로 RFC 4347에 정의되어 있다.

데이터그램은 UDP와 같으므로 암호화한 UDP 가 된다.

HTTP의 기반이 되는 TCP는 시퀀스 번호 등을 가지고 있어, 패킷 순서가 바뀌거나 패킷을 유실했을 대 정렬하거나 재전송을 요청해 올바른 데이터를 수신하려고 한다.

UDP 자체는 재전송이나 정렬을 하지 않아 신뢰성이 떨어지는 대신에 속도가 빠르다.

재전송 처리 등이 일어나지 않는, 다시말해 느려지지 않는 특성이 WebRTC에서는 중요하다.

**WebRTC 미디어 채널**

음성과 영상을 전송하는 `WebRTC` 미디어 채널은 `DTLS` 위에 `SRTP` 라는 프로토콜을 사용한다.

TLS 상의 프로토콜은 이름 앞에 `S` 가 붙는 것이 많다.

`SRTP`도 그중 하나로 보안된 실시간 프로토콜(Secure Real-time Transport Protocol) 라는 뜻이다.

음성과 영상을 스트리밍할 때 정보가 바뀌어 버리면 문제가 되므로, RTP는 순서 정렬 기능만을 UDP엣 추가했다.

재전송 처리는 스트리밍에선 부하가 커서 보증하지 않는다.

음성과 영상에서는 소리가 튀거나 프레임 드롭으로 처리 된다.

`RTCPeerConnection` 접속이 확립된 시점에서 미디어 채널의 통신 준비는 완료된다.

### RTCDataChannel

데이터 채널은 음성 이외의 데이터를 P2P로 송수신하기 위한 것이다.

활용 사례에느 ㄴ파일을 대화 상대에게 보내거나 통신대전 게임에서 컨트롤 정보를 공유하는데 사용했다.

**SCTP**

데이터 채널은 `DTLS` 위에 SCTP(Stream Control Transmission Protocol)라는 프로토콜을 올렸다.

데이터 채널의 활용 사래는 영상, 음성과 비교해 다소 차이가 있다.

데이터 파일의 데이터가 중간에 사라지거나 일부 누락되면 곤란한 경우가 있는가 하면, 그보다는 응답성이 손상되는 쪽이 문제가 되는 경우도 있다.

그래서 SCTP는 성능과의 ㅡ레이드오프를 통해 신뢰성을 설정할 수 있게 되어 있다.

UDP 쪽에 맞출지 TCP 쪽에 맞출지 선택할 수 있게 되어 있다.

### ORTC

리눅스 버전 스카이프 WebTRC를 사용해 구현됐다.

엣지 브라우정용 스카이프에서 사요된 것은 ORTC(Objct-Real0-Time Communication)이다.

`ORTC` 는 `WebRTC` 으 ㅣAPI를 더욱 다음고 또한 부족햇던 다양한 기능을 추가하련느 활동이다.

`WebRTC` 를 출발점으로 하므로, 백엔드인 통신 레이어(DTLS의 SRTP)와 코덱 등은 `WebRTC`와 완전히 같다.

SDP를 제외하거나 API를 정리하고 멀티스트림 지원이나 단말 성능에 따른 동영상 품질을 바꾸는 등 더  세밀한 조작을 할 수 있도록 저수준 기능을 확장하는 내용이었다.

## HTTP 웹 푸시

HTTP 웹 푸시는 웹사이트에 스마트폰 애플리케이션과 같은 알림 기능을 제공하는 구조다.

HTTP 웹 푸시는 기능의 특성상 브라우저가 그 시점에서 시작되지 않았거나 오프라인이라도 사용자에게 알림을 보낼 수 있어야 한다.

`Service Worker` 으로 브라우저가 없어도 통신이 가능하다.

`Service Worker` 프론트엔드 브라우저의 HTML 렌더링 기능으로 웹 서버와 프론트엔드 중간에 있는 프록시 서버와 같은 존재다.

그러나 `Service Worker`는 항상 실행되는게 아니라 `addEventListnner()` 이벤트 핸들러를 등록해두면, 필요할 때만 실행되어 처리한다.

웹 푸시는 현재 푸시 서비스와 연계해 구현한다.

각 서비스에서 서비스 ID와 전송용 키를 가져온다.

수신할 떈 ID가 전송할 때 ID와 키가 필요하다.

우선 브라우저를 시작할 때 푸시 서비스르 이용하는 등록을 한다.

푸시 알림은 사용자의 허가가 필요한 옵트인 방식으리므로 기본으로 활성화되지 않고 사용자로부터 하거를 얻어야 한다.

사용자가 허가하면, 푸시 서비스에 등록한다.

활성화되면 `Service Worker`로 등록한 push 이벤트 알림을 받을 수 있다.

애플리케이션 서버에서 푸시 알림을 보내려면 푸시 서비스의 API를 사용해야한다.

브라우저는 알림을 수신할지 사용자에게 확인한다. 

사용자가 승인할 때 푸시 서비스에 가입하고, 키도 이때 만들어진다.

키 정보는 푸시 서비스가 브라우저를 식별하는데 사용한다.

다음으로 브라우저는 푸시 전송에 필요한 키를 애플리케이션 서버에 보낸다.

그러면 애플리케이션 서버는 특정 브라우저로 보낼 수 있게 된다.

서버가 통지할 때는 브라우저에서 보내온 키를 이용해 푸시 서비스에 요청한다.

푸시 서버와는 HTTP로 통신한느 사양으로 되어 있다.

크롬이 이용하는 푸시 서비스는 GCM(Google Cloud Message)와 FCM(Firebase Cloud Message)을 기반으로 한다.

따라서 각각 애플리케이션을 등록해야하고 애플리케이션 별로 전용 키 및 URL이 제공된다.

파이어폭스가 이용하는 오토푸시(autopush)는 푸시 서비스와의 통신에 RFC에 기술된 HTTP 통신이 아닌 웹소켓을 사용한다.

이 부분은 브라우저 벤더가 각 브라우저용으로 독자적으로 푸시 서비스를 준비하는 상황이므로, 브라우저의 인터페이스는 표준을 따를 필요가 없다고 판단한 것일거다.

웹소켓은 실시간성이 높은 양방향 통신을 제공하므로 푸시 알림 응답 속도가 빨라져 사용자에게 이익이 된다.

또한 오토푸시는 GCM/FCM, 애플 기기를 위한 APNS(Apple Push Notfication Service)로의 브릿지에도 대응한다.

## 마치며

- HTTP/2는 증가하는 콘텐츠에 대응할 수 있도록 병렬 수를 늘리거나 이전에는 압축되지 않던 헤더를 압축해 크기를 줄이고 우선순위를 설정할 수 있게 됐다. 또한 콘텐츠를 미리 보내는 서버 푸시 등 HTTP/1에서는 처리할 수 없는 기능이 많이 포함됐다.
- Fetch API는 Service Worker와 함께 사용되는 HTTP 요청용 API로서 추가됐지만, 옵션을 이용해 다양한 케이스에 대응할 수 있게 되고 보안 모델도 HTTP 표준과 같은 개념으로 구현됐다.
- server-sent events 는 서버로부터 클라이언트에 정보를 보내기 위한 기술이다. 청크 전송 기술을 바탕으로한다.
- 웹소켓은 서버0클라이언트에서 고속 양방향 통신을 하는 기술이다. 실시간성 높은 채팅이나 게임의 인프라로 사용된다.
- 서버를 사용하지 않는 P2P 통신이나 브로드캐스트를 실현하는 것이 WebRTC이다. 영상 통화 및 데이터 통신을 지원한다.
- 웹 애플리케이션화에서 중요한 기능이 웹푸시다. 웹 푸시는 server-sent events와 웹소켓, WebRTC처럼 TCP와 UDP 등 네으퉈크 세션이 유지되지 않아도 통지한다.

# 8장. HTTP/2의 시맨틱스: 새로운 활용 사례
이 장에서는 검색엔진과 소셜 미디어가 이해하는 프로토콜을 중심으로 소개한다.

## 8.1 반응형 디자인

2장에서 소개한 것처럼 이전에는 유저 에이전트 정보를 보고 모바일용 콘텐츠를 배포하는 방식이 주류였다.

현재 오류가 더 적고, 확실하게 작은 화면에 적합한 레이아웃 또는 태블릿의 가로와 세로에 각각 최적인 레이아웃 등 다양한 경우 적절하게 표현할 수 있도록 하는 것이 반응형 디자인이다.

## 8.2 시맨틱 웹

시맨틱 웹은 웹에서 표면적인 `텍스트, 문서` 가 아니라 `의미`를 다룰 수 있게 해서 웹의 가능성을 더욱 넓히고자 하는 운동이다.

시맨틱 웹이서 `의미를 다룬다`는 것은 페이지에 포함된 정보를 분석해서 정보를 집약하거나 검색하는 과정을 사람손에 거치지 않게 하는 것이다.

그러기 위해선 데이터를 정규화해 프로그램에서 접근 가능한 형태로 만들어야한다.

### 8.2.1 RDF

최초의 시작은 XML 등을 이용해 의미를 기술하려는 범용적인 접근에서 출발했다.

이때 제안된것이 RDF(resource Description Framework)라는 형식이다.

RDF는 URI로 식별되는 엔티티 간의 관계성을 기술한다.

시맨틱 웹에서는 설명할 대상 요소를 `주어` , 주어의 속성을 `목적어`, 그 관계를 `술어` 라고 한다.

스키마에 스키마가 포함되어 있어, 상세한 정보를 알기 위해선 링크한 곳까지 읽어 들이지 않으면, 데이터 실체의 전모를 파악하기 상당히 어렵다.

이는 XML 기반 데이터 형식의 흔한 특징이다.

시맨틱 웹 세계에서는 `주어`, `술어`, `목적어` 라는 관계성 속에서 다양한 요소를 링크하여 세계의 의미를 기술하려한다.

이  `주어`, `술어`, `목적어` 는 가장 중요한 키워드 이고 트리플이라 불린다.

이 최초의 형식은 현재는 RDF/XML 이라 불린다.

웹사이트는 HTML로 기술하므로 그대로 RDF를 삽입할 수 없었고. 페이지 외부에 리소스로서 둘 필요가 있었다.

### 8.2.2 더블린 코어

더블린 코어(Dublin Core)는 RFC 5013과 ISO등에서 정의된 메타데이터 집합이다.

오픈 eBook이라는 규격에 포함되는, 파일 목록과 서적 정보를 담은 OPF(Open eBook Pacage Format)이라는 EPUB 파일의 핵심 파일이 있다.

모든 EPUB 파일에도 반드시 포함되는 .opf 파일에는 `dc:` 같은 태그가 있다.

`dc:` 로 시작하는 메타데이터가 더블린 코어의 구성요소다.

이처럼 서적, 음악 등 저작물에 범용적으로 사용할 수 있는 메타데이터로 되어 있다.

### 8.2.3 RSS

RSS는 웹사이트의 업데이트 기록 요약에 관한 배급 형식이다.

RSS는 블로그나 웹사이트를 업데이트하면 블로그 엔진과 콘텐츠 관리 시스템이 자동으로 업데이트 한다.

웹사이트를 브라우저로 돌아다니지 않고도 전용 리더가 등록한 RSS의 업데이트를 확인하고, 읽지 않은 항목을 모아서 효율적으로 읽을 수 있게 해준다.

### 8.2.4 마이크로포맷

HTML 태그와 클래스를 사용해 표현해 간다.

마이크로포맷은 HTML에 삽입하므로 RDF보다 이동성이 높다.

단 CSS 클래스 이름으로 충돌하기 쉬운 이름을 사용하므로, CSS 이름 충돌로 레이아웃이 무너지거나 스타일을 대폭 수정해야하만 하는 위험성이 있다.

### 8.2.5 마이크로데이터

마이크로데이터는  WHATWG, W3C 등에서 논의돼 온 HTMl에 삽입 가능한 시맨틱 표현 형식이다.

W3C에 정의되어있다.

마이크로데이터도 마이크로포맷과 마찬가지로 어휘집을 가지고 있으며, 현재도 검색 엔진이 지원한다.

기존 HTML 속성과 충돌하지 않도록 고유 속성을 사용한다.

사용하는 속성은 `itemscope`, `itemtype`, `itemprop` 세 종류다.

## 8.3 오픈 그래프 프로토콜

오픈 그래프 프로토콜은 소셜 네트워크에서 사용되는 메타데이터다.

오픈 그래프 프로토콜이 설정된 웹사이트의 URL을 SNS 등에 붙여 넣으면 기사의 일부가 인용되고 이미지도 표시된다.

오픈 그래프 프로토콜을 사용

    <html xmls:og="http://ogp.me/ns#">

다음으로 SNS를 위한 정보를 메타 태그로서 기술한다.
...
오픈 그래프 프로토콜과 비슷한 트위터 카드가 있다.

트위터 카드도 트위터에 링크를 붙였을 때 방문할 페이지의 내용을 일부 트위터에 보이게 함으로써 클릭룰을 올리려는 것이다, 

트위터 카드는 오픈 그래프 프로토콜을 기반으로 트위터에 대한 정보를 추가한 것이라고 할 수 있다.

## 8.4 AMP

`AMP(Accelerated Mobile Pages)`는 특히 휴대 기기에서 웹페이지의 로딩 속도를 `빠르게` 하는 방법이다.

모바일 사이트에서는 우선적으로 표시되고, `'AMP'` 마크도 붙어 있어 사용자가 빠르게 표시되는 페이지를 선택하기 쉽게 되어있다.

또한 필요한 정보가 설정된 페이지라면, 캐러셀(carousel)형식으로 기사의 요약과 함께 목록이 표시된다.

그러나 어던 페이지에서도 속도가 빨라지는 것은 아니고, 적당한 장소가 정해져 있다. 

구글 블로그에는 다음과 같이 적혀 있다.

    AMP은 뉴스, 요리법, 영화 목록, 제품 페이지, 리뷰, 동영상, 블로그 등 모든 유형의 정적 웹 콘텐츠에 적합하다.
    
    한편 경로 안내, 전자 메일 또는 소셜 네트워크 등 동적 또는 상호작용을 중시한 단일 페이지 앱에는 별로 효과가 없다.

뉴스 사이트의 경우 톱 페이지는 그대로고, 각 기사만  `AMP` 화할 수도 있다.

현대 웹사이트는 복잡한 자바스크립트를 구사해 구축된다. 그것을 평준화해서 모바일 전용으로 경량 페이지를 만드는 것이 `AMP` 의 출발점이다.

`APM` 은 적어도 만드는 방법에 엄격한 제약을 부과함으로써 `만드는 방법이 나빠서 표시가 지연되는 것` 을 방지한다.

`Ajax` 로 나중에 콘텐츠를 가져올 수도 없어 정적 콘텐츠만 볼 수 있다.

`AMP` 가 빠른 비밀은 콘텐츠 제작 방식뿐만아니라 배포 방식에도 있다.

검색 엔진을 위해 정보를 수집하는 프로그램을 `크롤러` 또는 `스파이더` 라고 부른다.

크롤러가 콘텐츠를 발견했을 때 `AMP` 이거나 <link> 태그에 AMP 페이지가 있는 것을 인식하면, 모든 내용을 구글의 배포 전용 고속 캐시 서버(CDN: 콘텐츠 전송 네트워크)에 복사한다.

CDN에서는 URL이 `[https://www.google.co.kr/app/s](https://www.google.co.kr/app/s)` 로 시작하는 같은 오리진 서버에서 콘텐츠가 제공된다.

예를들어 기사가 `[news.example.com/finance](http://news.example.com/finance)` 라고 하면, URL은 `[https://www.google.co.kr/amp/s/news.example.com/finace](https://www.google.co.kr/amp/s/news.example.com/finace)` 가 된다.

같은 서버이고 자바스크립트가 제한되어 있으면, 스크립트도 캐시되므로 AMP의 HTML이 다운로드된 순간에 표시할 수 있을 것이다.

AMP 사양에는 없지만, 구글 서버에서는 실제로 더 공격적인 최적화가 이루어지고 있다.

AMP를 기술적으로 보면 다음 네 가지 고속화 기법으로 구성된다.

- 페이지 구성을 고정화함으로써 페이지 로딩의 고속화를 달성했다.
- 페이지 구성이 고정화되므로 CDN 지원이 쉬워졌다.
- 콘텐츠를  CDN 쪽에 업로드함으로써 자바스크립트 파일을 캐시에 올리기 쉬워졌다.
- 콘텐츠를 서버 쪽에 업로드할 때 태그를 수정해 고속화가 이루어졌다(샤양 외).

## 8.5 HTTP 라이브 스트리밍에 의한 동영상 스트리밍 재생

HTTP 라이브 스트리밍(HTTP Live Streaming HLS)은 애플이 2009년에 제창한 동영상스트리밍 재생 방식이다.

현재는 모바일의 사파리, 크롬과 데스크톱의 사파리, 엣지에서 지원되고 있다.

모바엘에서는 사실상의 표준이다.

- 콘퍼런스의 (거의) 실시간 중계 등의 스트리밍에 사용할 수 있다.
- 회선의 속도에 맞춰 적절한 해상도의 동영상을 선택할 수 있다.
- 자막과 음성을 전환할 수 있다.
- 동영상 포맷으로는 지상파 디지털 방송에도 사용되는 MPEG2-TS(.ts)를 사용한다. 2016년 WWDC에서는 Fragmented MP4도 지원한다고 발표됐다.
- 사양에서 동영상 포맷을 지정하지 않았지만, 애플에서는 영상에 H,264, 음성에 AAC, MP3, AC-3을 지원한다.

### 8.5.1 HLS의 비디오 태그

우선 HLS를 사용한 HTMl의 예를 소개한다.

WWDC 2016에서 HLS를 소개한 세션의 페이지 `(https://developer.apple.com/video/play/wwdc2016/504)` 는 다음처럼 `<video>` 태그를 사용한다.

브라우저마다 지원하는 동영상 형식이 다르기 때문에 `<video>` 태그는 여러가지 소스를 기술할 수 있게 되어 있다.

### 8.5.2 마스터의 .m3u8 파일

`.m3u8` 파일은 파일 정보를 담은 텍스트 파일로, UTF-8 형식으로 된 재생 목록(m3u)이다.

`.m3u8` 파일에서 다른 .m3u8 파일을 참조할 수 있다.

### 8.5.3 자막 .m3u8 파일

자막 파일은 60초 단위로 잘려 있다.

루트 .m3u8 파일에 잘린 자막 파일을 묶는 .m3u8 파일이 링크되어 있다. 파일의 시작 부분을 인용한다.

예제의 파일은 WEBVTT 형식의 자막 스크립트다.

WEBVTT는 HLS 전용 포맷이 아니라 W3C가 정의하는 웹 표준의 하나다.

WEBVTT의 콘텐츠는 블록별로 나뉘어져 있으며, 각 블록에는 시작 시간과 종료 시간, 어느 위치에 어떤 텍스트를 표시할지 기재되어 있다.

### 8.5.4 동영상 파일

마스터 인덱스의 .m3u8이 참조하는 인덱스는 대체 인덱스라고 한다.

클라이언트는 파일에 적힌 순서대로 동영상을 로드해 표시한다.

생방송 즉시 주문형 비디오(요청한 타이밍에 동영상을 처음부터 보여준다)로도 사용하는 경우에는 위의 태그를 VOD로 설정한 다음, 끝에 #EXT-X-ENDLIST를 추가한다.

나중에 다시 보는 것을 가정하지 않고 계속 중계만 하는 생방송인 경우, 업데이트 시 재생이 끝난 동영상을 삭제해 항상 최신 동영상을 남겨둔다.

### 8.5.5 HTS의 장점과 된점

HLS의 장점

- 서버와 클라이언트의 통신에 특수한 프로토콜이 아니라 전 세계 라우터가 지원하는 HTTP를 사용하는 점

특수한 프로토콜은 전용 서버를 설치하고 통신할 포트를 열어야 할 필요가 있다.

HLS는 HTTP이므로 일반 웹 서버와 동일한 서버를 사용할 수도 있고, 콘텐츠 전송 네트워크 또한 사용할 수 있다.

서버 측에서 필요한 설정은 MIME 타입 및 캐시 유효 기간 두 가지 뿐이다.

생방송의 경우는 대체 인덱스 파일을 빈번하게 다시 로드한다.

캐시되면 새로운 동영상이 추가돼도 클라이언트에서 눈치챌 수 없게 된다.

주문형 비디오로 사용하는 경우에는 캐시 문제가 없다.

단점으로는 `스트리밍` 라고 자칭하지만, 실체는 점진적 다운로드 방식이라는 점이다.

청크별로 다운로드가 끝나지 않으면, 재생이 시작되지 않으므로 지연이 발생한다.

12초의 청크로 회선이 허용하는 최대 화질로 영상을 재생하려면  `12초 X 2 + 인코딩 시간` 만큼 지연이 발생한다.

가장 큰 단점은 지원되지 않는 환경이 많다는 점이다.

특히 데스크톱에서의 지원이 좋다고는 할 수 없다.

태스크톱의 경우 플래시나 실버라이트가 일반적인 동영상 재생 도구로 사용됐으므로, 그때까진 그다지 중요도가 높지 않기 때문이라고 생각한다. 다만 플래시도 이미 종료 카운트다운이 시작됐다.

그 밖에도 반년마다 내용이 개정되고, 동영상 형식이 고정되어 있으며, 콘텐츠 보호 DRM에 제약이 있는 점도 단점으로 들 수 있다.
