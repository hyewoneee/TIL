...
싱글 머신에서 멀티 프로세스와 스레드를 실행하기에 충분하지 않을 때도 잇다. 이번 절에서는 싱글박스(하나의 머신)와 멀티박스 병행성 사이를 연결해주는 브리지(다리)에 대해 다룬다.

Redis는 데이터베이스 역할을 한다. 여기서는 병행성에 대해 다룬다.

큐를 만들 수 있는 빠른 방법은 Redis의 리스트다. 

Redis 서버는 하나의 머신에서 실행한다.
...
하나 이상의 클라이언트 워커는 리스트를 감시하며, **블로킹 팝**(pop)연산을 수행

리스트가 비어 있는 경우에는 메시지를 기다린다.

메시지가 도착하자마자 첫 번째 워커가 메시지를 처리한다.

프로세스 마지막 **센터널(sentinel 한 단위 정보의 시작 또는 끝 또는 특별한 지점을 나타내는 값**)값을 사용하지 않는다면 다음과 같이 더 많은 프로그램 로직을 추가해야한다.

- 최대 접시 수(센터널 역할을 함)가 넘어갔을 때 작업을 종료한다.
- 특정 **구간 외**(데이터 스트림이 아닐 때) 프로세스 간 통신을 한다.
- 특정 시간 간격 내에 새로운 데이터가 없다면 작업을 종료한다.

### 큐를 넘어서

조금 더 나아가서 설거지 프로세스 라인이 중단될 가능성에 대해 생각해보자

**실행 후 잊어버리기(fire and forget)**

워커를 전달할 곳에 아무것도 없더라도, 전달한 후 그 결과에 대해 걱정하지 않는다.
...
느린 워커의 속도가 빠른 워커의 속도를 따라갈 수 없을 때, 빠른 워커의 속도를 조절한다.

실제 시스템에서는 워커를 필요한 수만큼 잘 유지해야한다. 

일부 워커 프로세스가 최신 메시지를 **팝**해서 작업 리스트에 추가하는 동안, 새 작업을 **대기** 리스트에 추가해야한다.

이렇게 하면 어떤 작업이 실패했는지 혹은 얼마나 기다렸는지 알 수 있다.

이것을 Redis를 사용하여 스스로 구현하거나 누군가 이미 작성하고 테스트한 시스템을 사용할 수 있다.

외부 관리가 추가된 파이썬 기반의 큐 패키지들

**celery**

이 특별한 패키지는 접해볼 가치가 있다.

이번 장에서 다뤘던 `multiprocessing, gevent` 등을 사용하여 동기 또는 비동기 분산작업을 실행

**thoonk**

Redis에서 작업 큐와 **발행-구독**(publish-subscribe) 모델을 제공하기 위한 패키지

**rq**

작업 큐를 위한 Redis 기반의 파이썬 라이브러리

**Queues**

파이썬을 포함해서  여러가지 언어를 기반으로 큐 소프트웨어에 대한 정보를 제공

### 네트워크

이 절에서는 공간을 포함한 네트워킹 및 분산 컴퓨팅에 대해 살펴보자

### 패턴

가장 일반적인패턴은 **요청-응답(동기적)** 패턴으로,

**클라이언트-서버** 패턴으로 알려져 있다. 

- 클라이언트는 서버의 응답이 올 때까지 기다린다.
- 웹 브라우저 또한 HTTP 요청을 만들어서 웹 서버의 응답을 기다리는 클라이언트다

**푸시(push)** 또는 **팬아웃(fanout)** 패턴

데이터를 프로세스 풀(pool)에 있는 사용 가능한 워커로 전송

- 예를 들어 로드 밸런서 뒤에는 웹 서버가 있다.

푸시의 반대는 **풀(pull)** 또는 **팬인(fanin)**이다.

하나 이상의 소스로부터 데이터를 받는다.

- 예를 들어 멀티 프로세스에서 텍스트 메시지를 받아서, 하나의 로그 파일에 작성하는 로거(logger)가 있다.

패턴은 **발행-구독(pulish-subscribe)**하는 라디오나 텔레비전의 방송과 유사

이 패턴은 발행자(publisher)가 데이터를 전송한다.

간단한 발행-구독 시스템의 모든 구독자(subscriber)는 데이터의 복사본을 받는다.

구독자는 자주 특정 타입에 대한 데이터(토픽 topic)에 관심이 있음을 표시할 수 있다.

그리고 발행자는 단지 토픽을 전송한다.

푸시 패턴과는 달리. 여러 구독자는 주어진 데이터 조각을 받는다. 

토픽에 대한 구독자가 없는 경우, 데이터는 무시된다.

### 발행-구독 모델

큐가 아닌 브로드캐스트(broadcast)다.

하나 이상의 프로세스가 메시지를 발행한다.

각 구독자 프로세스는 수신하고자 하는 메시지의 타입을 표시

각 메시지의 복사본은 타입과 일치하는 구독자에 전송된다.

주어진 메시지는 한 번 또는 여러 번 처리되거나, 아예 처리되지 않을 수도 있다.

각 발행자는 단지 브로드캐스팅만 할뿐, 누가 구독하는지 알지 못한다.

### Redis

Redis를 사용하여 발행-구독 세스템을 빠르게 구형할 수 있다.

구독자는 토픽, 값과 함꼐 메시지를 전달하고, 구독자는 수신받고자 하는 토픽을 말한다.

원하는 만큼 구독자와 발행자를 가질 수 있다.

메시지에 대한 구독자가 없다면, Redis 서버로부터 메시지가 사라진다.

구독자가 있다면, 모든 구독자가 메시지를 받을 때까지 메시지는 서버에 남는다.

### ZeroMQ

ZeroMQ는 중앙 서버가 없으므로, 각 발행자는 모든 구독자에게 메시지를 전달한다.
...
pika는 RabbitMQ를 위한 파이썬 API다.

**pypi.python.org**

pubsub를 입력하여 pypubsub와 같은 패키지를 찾아본다.

**pubsubhubbub**

구독자와 발행자와 함께 콜백을 등록할 수 있게 해준다.

### TCP/IP

인터넷은 커넥션을 맺고, 데이터를 교환하고, 커넥션을 종료하고, 타임아웃을 처리하는 등의 방법에 대한 규칙에 의거한다.

이를 **프로토콜**이라고 하며, **계층(layer)**으로 정렬되어 있다.

계층의 목적은 일을 처리하는 데 있어 새로운 여러 가지 대안을 허용하기 위함이다.

위 계층과 아래 계층을 처리하는 규칙을 따른다면, 한 계층에서 원하는 모든 작업을 수행할 수 있다.

가장 낮은 계층에서는 전기 신호를 처리한다.

각 계층은 층층이 쌓여있고, 중간에 있는 계층은 네트워크의 위치와 데이터 흐름의 **패킷**(packet 청크(chunk)을 명시하는 IP 계층이다.

IP 계층에는 네트워크 위치 사이에서 바이트를 이동하는 방법을 기술하는 다음 두가지 프로토콜이 있다

**UDP(User Datagram Protocol(사용자 데이터 프로토콜)**

짧은 데이터 교환에 사용된다.

**데이터그램**은 한 단위로 전송되는 작은 메시지다.

**TCP(Transmission Control Protocol(전송 제어 프로토콜)**

수명이 긴 커넥션에 사용된다.

TCP는 바이트 **스트림**이 중복 없이 순서대로 도착하는 것을 보장한다.

UDP 메시지는 응답 메시지(ACK) rk djqtek.

그래서 메시지가 목적지에 잘 도착했는지 확인할 수 없다.

TCP는 송신자와 수신자 사이의 커넥션을 보장하기 위해 핸드셰이크(handshake)를 설정

웹과 데이터베이스 서버 등 우리가 소통하는 대부분의 인터넷은 IP 프로토콜의 맨 위에서 실행되고 있는 TCP 프로토콜이다(간단히 TCP/IP 라고함).

### 소켓

네트워크 프로그래밍의 가장 낮은 수준은 C언어와 유닉스 운영체제에서 빌려온 **소켓**을 사용

클라이언트는 UDP 데이터그램 안의 한 문자열을 서버로 보낸다.

서버는 문자열이 담긴 데이터의 패킷을 반환한다.

서버는 우체국과 우체국 사서함 같이 특정 주소와 포트를 청취(listen)해야 한다.

클라이언트는 메시지를 전달하고 서버로부터 어떤 응답을 받기 위해 이 두 값을 알아야한다.

먼저 한 프로세스에서 다른 프로세스로 작은 데이터를 전송한 후, 데이터를 다신 보낸 사람에게 반환해보자.

먼저 서버 프로그램을 작성한 뒤, 클라이언트 프로그램을 작성한다.

각 프로그램은 시간을 출력하고 소켓을 연다.

서버는 소켓 연결을 청취하고, 클라이언트는 서버에 전송할 소켓을 작성한다.

UDP는 한 청크(chunk)에 데이터를 보낸다.

데이터 전송을 보장하지 않는다.

UDP를 통해 여러 메시지를 보낼 경우, 순서 없이 도착하거나 모두 도착하지 안흥ㄹ 수 있다.

UDP는 빠르고, 가볍고, 신뢰할 수 없고, 비 연결형이다.

우리는 왜 TCP를 선호하는가?

TCP는 웹과 같은 수명이 긴 커넥션에 사용한다.

TCP는 데이터를 보낸 순서대로 전달하고 전송에 문제가 생기면 다시 보낸다.

TCP는 소켓의 여러 호출을 통해 클라이언트와 서버의 커넥션을 유지하고, 클라이언트의 IP 주소를 기억한다.
...
- TCP를 통해 전체 메시지를 전달하기 위해 세그먼트(segment) 로 부터 전체 메시지를 재구성하기 위한 몇 가지 추가 정보가 필요하다(고정 메시지 크기(바이트), 메시지 전체 크기, 구분자).
- 메시지는 유니코드 텍스트 문자열이 아닌 바이트이기 때문에 파이썬 바이트 타입을 사용해야 한다.

### ZeroMQ

**강력한 소켓**(sockets on steroids)라 불리는 ZeroMQ 소켓은 다음과 같은 일을 수행
...
다음 목록에 기술되어 있는 기본적인 '레고 조각' 들은 ZeroMQ의 소켓타입이다.
...
**벤틸레이터**(ventilator)라 불리는 또 다른 네트워킹 패턴은 PUSH 소켓을 사용하여 비동기 작업을 처리하고, PULL 소켓을 사용하여 결과를 수집한다.

ZeroMQ의 중요한 특징은 소켓이 생성될 때 소켓의 커넥션 타입을 바꿔서 스케일 업(scale up)과 스케일 다운(scale down)을 한다는 것이다.

- tcp: 하나 이상의 머신에서 프로세스 간 통신
- ipc(Inter-Process Communication): 하나의 머신에서 프로세스 간 통신
- inproc(IN-PROcess (inter-thread) Communication):
    - 한 프로세스에서 스레드 간 통신
    - 락(lock) 없이 스레드 간 데이터를 전달하는 방법

capy

웹 API를 디버깅하거나 일부 보안 문제를  추적해야 할ㄷ 대 네트워킹 스트림을 잡아서 바이트를 자세히 살펴볼 필요가 있다.

`scapy` 라이브러리는 패킷을 분석하기 위한 유용한 파이썬 도구다.

C 프로그램보다 코드를 작성하고 디버깅하는 것이 훨씬 쉽다.

`scapy`는 실제로 패킷을 만들고, 분석하기 위한 작은 언어다.

### DNS

컴퓨터는 `85.2.101.94` 와 같이 숫자로 된 IP 주소를 갖고 있다. 

DNS(Domain Name System)는 분산된 데이터 베이스를 통해 IP주소를 이름으로 바꾸거나, 그 반대를 수행하는 아주 중요한 인터넷 서비스다.

웹 브라우저에서 사이트를 입력할 때 갑자기 `호스트 검색중..` 과 같은 메시지를 본다면, 아마 인터넷 연결이 안 됐거나 DNS 검색 실패를 의심할 수 있다.

### 파이썬 이메일 모듈

표준 라이브러리는 다음 이메일 모듈을 포함한다.

**smtplib 모듈**

- SMTP(Simple Mail Transfer Protocol)를 통해 이메일 전송
...
- POP3(Post Office Protocal 3)를 통해 이메일 읽기

**imaplib 모듈**

- IMAP(Internet Message Access Protocal)을 통해 이메일 읽기

### 원격 프로세싱

로컬 머신처럼 다른 머신의 코드를 실행할 수 있다.

만약 싱글 머신에서 실행 공간이 부족하다면, 원격 프로세싱으로 그 공간을 확장할 수 있다.

머신의 네트워크는 더 많은 프로세스와 스레드를 접근하게 해준다.

### 원격 프로시저 호출

RPC(Remote Procedure Call 원격 프로시저 호출)는 평범한 함수처럼 보이지만, 네트워크를 통해 원격에 있는 머신을 실행한다.

URL 또는 요청 바디(body)에서 인코딩된 인자들과 RESTfull API를 호출하는 대신 RPC 함수를 호출

RPC 클라이언트에서는 다음과 같은 일이 발생한다.

1. 함수의 인자를 바이트로 변환(**마샬링** marchalling 또는 **직렬화** serializing 또는 그냥 **인코딩**이라 부름)
2. 인코딩된 바이트를 원격 머신으로 전송

원격 머신에서는 다음과 같은 일이 발생한다.

1. 인코딩된 요청 바이트 수신한다.
2. 바이트를 수신한 후, RPC 클라이언트는 다시 원래의 데이터 구조(혹은 다른 두 머신 사이의 하드웨어와 소프트웨어가 서로 다른 경우 그에 맞는 구조) 로 바이트를 디코딩 한다.
...
서버 측에서는 서버 프로그램을 구동하고, 몇몇의 바이트 전송과 인코딩/디코딩 메서드와 함께 RPC에 연결하여 일부 서비스 함수들을 정의하고, 신호를 받기 위한 **RPC 서버를 구동**한다.

클라이언트는 서버에 연결하여 RPC를 통해 함수를 호츨한다.

### fabric

`fabric` 패키지는 원격 또는 로컬 명령이나 파일 업로드 및 다운로드를 sudo 권한의 사용자로 실행한다.

원격 머신의 프로그램을 실행하기 위해 `fabric` 패키지는 SSH (텔넷을 대체하는 암호화된 텍스트 프로토콜)를 사용한다.

파이썬에서 소위 **fabric 파일**이라 부르는 곳에서 함수를 작성하고, 로컬 혹은 원격으로 함수를 실행하는지 여부를 작성한다.

`fabric` 프로그램을 실행할 때, 어떤 원격 머신을 사용할지, 어떤 함수를 호출할지 입력한다.

### SaltStack

`SaltStack` 은 원격 실행의 구현으로 시작했지만, 본격적인 시스템 관리 플랫폼으로 성장했다.

`SSH`가 아닌 `ZeroMQ` 를 기반으로 한 `SaltStack` 은 수천 개의 서버를 확장할 수 있다.

### 빅데이터와 맵리듀스

개발자들은 네트워크가 연결된 머신에서 데이터를 분산하여 분석하는 것이 개별적인 머신에서 수행하는 것보다 더 빠르다는 것을 발견했다.

단순하게 알고리즘을 사용하여 대규모의 분산된 데이터가 전반적으로 더 잘 작동한다는 것을 실제로 확인했다.

그중 하나가 바로 **맵리듀스**(MapReduce)다.

이는 여러 머신에서 계산을 수행하여 결과를 수집한다.

맵리듀스는 큐 작업과 비슷하다.

구글은 이 결과를 논문으로 발표한 후, 야후에서 **하둡**(Hadoop)이라는 자바 기반의 오픈소스 패키지를 개발했다.

**빅데이터**는 '아주 큰 데이터를 머신에 맞게(data too big to fit on my machine'라는 문구의 의미가 담겨있다.

여기서 데이터는 디스크, 메모리, CPU타임을 초과하는 데이터를 의미.

하둡은 머신 ㄱ나에 데이터를 복사하여 맵과 리듀스 프로그램을통해 이들을 실행한다.

그리고 각 단계에서 디스크에 결과를 저장한다.

이 배치 프로세스는 느릴 수 있다. **하둡 스트리밍**이라 부르는 빠른 메서드는 각 단계에서 디스크에 기록할 필요 없이 프로그램을 통해 데이터를 스트리밍하는 유닉스의 파이프처럼 작동한다.

하둡의 라이벌인 스파크는 하둡보다 10배에서 100배 빠르게 실행하도록 설계도이었다.

스파크는 모든 하둡 데이터 소스와 포맷을 읽고 처리할 수 있다.

### 클라우드

개별적인 머신이 많을수록 시스템 장애가 자주 발생하며, 매우 일반적이다.

서비스를 수평 확장하고, 데이터를 다중으로 저장할 필요가 있다.

네트워크가 싱글 머신처럼 동작한다고 가정할 수 없다.

피터 도이치(peter Deutsch)에 의하면 분산 컴퓨팅의 8가지 착오는 다음과 같다.

- 네으퉈크는 신뢰할 만하다.
- 지연시간(latency)는 0이다.
- 대역폭(bandwidth)은 무한하다.
- 네트워크는 안전하다.
- 토폴리지(topology)는 바뀌지 않는다.
- 한 명의 관리자(administrator)가 있다.
- 전송비용은 0이다.
- 네트워크는 동일(homegeneous)하다.

이러한 복잡한 분산 시스템을 구축하기 위해 많은 작업과 다양한 도구 세트가 필요하다.

분산 시스템을 구축하는 대신 **클라우드**의 서버를 임대할 수 있다.

- 이 모델을 적용함으로써 유지보수를 다른 누군가의 문제로 돌릴 수 있으며 사용자들에게 제공하고 싶은 서비스에만 집중할 수 있다.
- 웹 대시보드와 API를 사용하여 필요한 구성의 서버를 쉽고 빠르게 돌릴 수 있다.
- 즉, 서버를 탄력적(elastic)으로 운영할수 있고, 서버의 상태를 모니터링할 수 있으며, 트래픽이 주어진 임계치를 초과하는 경우 경고 알림을 받을 수 있다.
...
표준 라이브러리에서는 두 개의 테스트  패키지를 제공

먼저 unittest를 살펴보자

테스팅의 기본은 정확한 입력값으로부터 원하는 어떤 출력값을 결정하는 것

그리고 테스트하려는 함수에 입력값을 제출한 후, 예쌍된 결과를 반환하는지 확인한다.

예상된 결과는 **어설션**(assertion)이라 부른다.

unittest 모듈은 작지만, 값과 원하는 클래스가 있는지 확인해주고, 

에러 발생 여부를 밝혀주는 등의 강력한 어설션 집합을 제공

### 테스트하기(2): doctest

표준 라이브러리의 두 번째 테스트 패키지는 doctest다.

doctest를 사용하여 docstring 내에서 코멘트와 함께 테스트 코드를 작성할 수 있다.

`>>>` 다음에 뭔가를 호출하고, 다음 줄에 결과가 있다. 이것은 대화식 인터프리터처럼 보인다.

대화식 인터프리터에서 테스트 하고나서, 테스트 파일에 그 결과를 붙여 넣어도 된다.

`-v` 옵션을 추가해서 자세하게 살펴보자

	python example.py -v

### 테스트하기(3): nose

unittest의 또 다른 대안은 써드파티 패키지 nose다

unittest에서 했던 것처럼 테스트 메서드를 포함하는 클래스를 생성할 필요는 없다.

함수의 이름에 test가 포함되어 있다면, 테스트가 실행된다.

`nosetests test_example.py`

### 에러 메시지 로깅

로그는 일반적으로 메시지가 축적되는 시스템 파일이다.

이는 타임스탬프나 프로그램을 실행하고 있는 사용자의 이름과 같은 유용한 정보를 고깅한다.

로그는 매일 **돌고**(이름이 바뀌고) 압축된다.

이렇게 함으로써 디스크를 절약하고, 로그를 정리한다.

로그는 특히 예외적인 문제가 발생할 때 유용하게 쓰인다.

프로그램이 죽은 원인이 되는 실제 라인을 보여주고, 그 이유를 알려주기 떄문이다.

표준 파이썬 라이브러리 모듈은 **logging**이다.

logging 모듈은 다음 개념을 포함한다.

- 로그에 저장할 **메시지**(message)
- 상위 우선 **레벨**과 그 함수들 debug(), info(), warn(), error(), critical()
- 모듈과 연결되는 하나 이상의 **로거**(logger) 객체
- 터미널, 파일, 데이터베이스 등으로 메시지를 전달하는 **핸들러**(handler)
- 결과를 생성하는 **포매터**(formatter)
- 입력을 기반으로 판단하는 **필터**(filter)

`debug()` 와 `info()` 함수는 유용하다.

특정 메시지(message)를 찾거나, 서버가 다운되기 전에 무슨일이 일어났는지 보기 위해 타임스탬프를 비교하는 등의 작업을 하기 위해 로그 파일의 특정 **레벨**값을 검색할 수 있다.

기본 우선 레벨은 **WARNING**이고 `basicConfig()` 함수를 사용해 기본 레벨을 설정할 수  있다.

	import logging
	loggin.basicConfig(level='DEBUG')

**핸들러**를 사용하여 다른 곳에 메시지를 출력할 수 있다.

	import logging
	logging.basicConfig(level='DEBUG', file_name='blue_ox.log')
	logger = logging.getLogger('bunyan')
	logger.debug("Where's my axe?")
	logger.warn("I need my axe?")

로그는 blue_ox.log 파일에 들어있다.

`filename`  인자로 `basicConfig()` 함수를 호출하면 파일 핸들러(FileHandler)를 생성하여 로거에 사용할 수 있도록 만들어준다.

`logging 모듈` 은 모니터 화면과 파일뿐만 아니라 이메일과 웹 서버 같은 곳에서도 메시지를 전송하는 핸들러를 제공한다.
...
**그것을 할 수 있는 분명한 한 가지 방법이 있어야 한다.**

**그 방법이 유일하다면 더 좋다**라고 말하고 있다.

때때로 그 방법은 명확하지 않아서 다른 대안과 비교할 필요가 있다.

예를 들어 리스틀르 생성하기 위해 for 문과 리스트 컴프리헨션 중에 리스트컴프리헨션이 두 배 더 빠르다.

일반적으로 컴프리헨션은 수동적인 구조보다 더 빠르다.

자신만의 아이디어로 코드를 더 빠르게 만들어보자

파이썬 코드를 최적화 하는 데 노력했지만 원하는 성능을 여전히 얻을 수 없는 경우, 몇 가지 옵션이 더있다.

**Cython**

파이썬과 C의 혼합버전이다.

일부 성능에 대한 어노테이션과 파이선 코드를 컴파일된 C 코드로 옮기기 위해 설계되었다.

이들 어노테이션은 일부 변수 타입 선언, 함수 인자. 혹은 함수 반환처럼 아주 작은 부분이다.

이러한 요소와 최적화된 루프는 코드를 몇천배 더 빠르게 만들어준다.

**NumPy**

NumPy는 속도를 위해 C로 작성된 파이썬 math 라이브러리다.

**C 익스텐션**

파이썬의 많은 부분과 표준 라이브러리는 속도를 위해 C로 작성

그리고 편의를 위해 파이썬으로 래핑(wrapping)되었다.

애플리케이션에서 이러한 후킹(hooking)을 쓸 수 있다.

C 익스텐션의 작성은 어렵지만, 성능 문제를 개선하는데 도움을 줄것이다.

**Pypy**

자바의 속도를 따라잡기 위해 일부 묘책이 적용된 새로운 파이썬 인터프리터

### 소스 컨트롤

**Mercurial**

파이썬으로 작성되었으며, 간단한 하위 명령으로 Mercurial 저장소로 부터 코드를 내려받고, 파일을 추가하고, 변경사항을 확인하고, 다른 소스로 부터 수정사항을 병합할 수 있어서 배우기 쉽다.

**Git**

Github는 수백만 개의 저장소를 가진 가장 큰 git 호스트
